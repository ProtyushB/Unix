import { configFor, moduleLabel } from './batchDetail.modules';
...
describe('configFor', () => {
  ...
  it('falls back to parlour rather than crashing on an unknown key', () => {
    // The module key arrives from AsyncStorage and can be stale after a business switch. Rendering
    // something beats a white screen.
...
describe('module divergence', () => {
  it('is EMPTY for both modules today, and this test is the tripwire', () => {
    // InventoryDto is one class; the module DTOs are empty subclasses. If someone adds a
    // pharmacy-only batch field server-side, this fails and points at the form that must follow.
