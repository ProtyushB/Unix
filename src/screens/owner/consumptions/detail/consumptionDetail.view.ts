 ...
 *   • quantity ≤ available RAW stock. The server refuses an over-draw with a message naming the
 *     shortfall, but the user has the number on screen already and should not have to round-trip
 *     for it.
 *   • `reason` — must satisfy `isConsumptionReason`. The service guards this too, and that guard is
 *     the last line rather than the first: a bad enum is an HTTP 500 with nothing readable in it.
 *   • `consumedAt` — not in the future, compared as `YYYY-MM-DD` against **IST** today
 *     (`todayIst()`), because that is the day the server evaluates against. A device in another
 *     timezone would otherwise disagree with the backend for a whole day at a time.
 *
 ...
 */
export function validateConsumption(_form: ConsumptionFormState): ValidationErrors {
  return {};
}
