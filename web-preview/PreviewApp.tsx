...
export function PreviewApp() {
  ...
  useLayoutEffect(() => {
    ...
    AsyncStorage.getItem('theme')
      .then((stored) => {
        if (alive && stored) setThemeId(stored);
      })
      .catch(() => {});
    return () => {
      alive = false;
  ...
  const groups = useMemo(() => {
    ...
    const filtered = REGISTRY.filter(
      (e) => !q || e.title.toLowerCase().includes(q) || e.id.toLowerCase().includes(q),
    );
    ...
    <div style={vertical ? styles.controlsV : styles.controls}>
      ...
      {vertical ? (
        ...
        <div style={styles.zoomBarV}>
          ...
          <button onClick={() => stepZoom(0.1)} style={styles.zoomBtn} title="Zoom in">
            +
          </button>
          <input
            type="range"
          ...
          />
          <button onClick={() => stepZoom(-0.1)} style={styles.zoomBtn} title="Zoom out">
            −
          </button>
      ...
      <button
        style={vertical ? styles.reloadV : styles.reload}
  ...
  return (
    <div style={styles.app}>
      ...
      <aside
        style={{
          ...styles.sidebar,
          width: sidebarOpen ? 280 : 'fit-content',
          minWidth: sidebarOpen ? undefined : 92,
        }}
      >
        <div
          style={{
...
const styles: Record<string, React.CSSProperties> = {
  ...
  btnPower: {
    ...
    height: 74,
    borderRadius: 3,
    background: 'linear-gradient(90deg, #23272f 0%, #5c6472 45%, #9aa2b0 100%)',
  ...
  btnVolUp: {
    ...
    height: 40,
    borderRadius: 3,
    background: 'linear-gradient(270deg, #23272f 0%, #5c6472 45%, #9aa2b0 100%)',
    boxShadow: '-1px 0 2px rgba(0,0,0,0.4)',
  },
  btnVolDown: {
    position: 'absolute',
